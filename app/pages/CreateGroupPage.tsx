...
export default function CreateGroupPage(): React.ReactElement | null {
    ...
    const onCreate = async () => {
        if (!groupName.trim()) {
            Alert.alert('Missing Group Name', 'Please fill in a group name');
            return;
        }

        setIsLoading(true);
        try {
            // Generate a simple group ID from the group name
            const groupId = groupName.replace(/\s+/g, '-').toLowerCase();
            const users = user?.username ? [user.username] : [];

            await createNewGroup(groupId, groupName, users);
            ...
            switch (err.message) {
                case "GROUP_ALREADY_EXISTS":
                    Alert.alert("Sorry", "A group with this name already exists. Please choose another.");
                    break;
                case "NETWORK_ERROR":
